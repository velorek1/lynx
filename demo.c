...
int i=0;
     ...
     for (i=0;i<d_columns;i++)
          write_ch(screen1, i,1, U_BOX2, B_BLACK, FH_BLACK);
     for (i=0;i<d_columns;i++)
          write_ch(screen1, i,d_rows, U_BOX2, B_BLACK, FH_BLACK);
     write_str(screen1,1,1,"DOUBLE SCREEN BUFFER DEMO - by v3l0r3k", B_WHITE, F_BLACK);
     //window1
     draw_window(screen1,2,3,25,12,B_BLACK,F_WHITE, B_CYAN, TRUE,TRUE,TRUE);
     ...
     write_str(screen1,3,8,"P: UPDATE ALL/XOR", B_BLACK, F_WHITE);
     write_str(screen1,3,9,"R: RED PAINT IF XOR", B_BLACK, F_WHITE);
     write_str(screen1,3,10,"+: DECREASE SPEED", B_BLACK, F_WHITE);
     write_str(screen1,3,11,"-: INCREASE SPEED", B_BLACK, F_WHITE);
     write_ch(screen1,24,11,'+',B_BLACK,FH_BLUE);
     //window2
   ...
   //animation     
     ...
     if (updateAll == FALSE){
        for (i=0;i<17;i++){
           ...
           if (redPaint == FALSE) 
		write_ch(screen1, whereX+i,whereY, oldchar.ch, oldchar.backColor, oldchar.foreColor);
           else 
		write_ch(screen1, whereX+i,whereY, oldchar.ch, B_RED, oldchar.foreColor);
       }
     }
